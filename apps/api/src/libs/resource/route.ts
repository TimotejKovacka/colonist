...
import { cleanupSchemaDefaults } from "../types/types.js";
import type { FastifyTypeboxInstance } from "../../utils/fastify.js";
import type { ResourceId } from "../types/resource-id.types.js";
import { assert } from "../../../../../packages/utils/src/assert.js";
import { CREATED_RESOURCE_ID_HEADER } from "../constants.js";
...
export function registerResourceRoutes<TResource extends BaseResource>({
  ...
  const ids = Object.fromEntries(
    resource.idsOrder.map((idKey) => [idKey, `:${idKey}` as ResourceId])
  );
  ...
  const createdAtIdsSchema = strictResourceSchema(
    resourceIdsModifiedAtSchema(resource)
  );
  ...
  for (const [methodName, methodSchema] of Object.entries(resource.methods)) {
