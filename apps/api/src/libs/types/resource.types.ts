...
export type ResourceMethodSchema = {
  description?: string;
  request: TObject;
  response: { [k in HttpCodes]?: TObject | TNull };
};
...
>({
  ...
  createId,
  idsOrder,
  authRoles, // auth id roles asserted during authorization
  body: bodyProperties,
  options = {} as Options,
  methods = {} as Methods,
  ...
  idsOrder: (keyof Ids)[];
  createId?: keyof Ids;
  authRoles: { [K in keyof Ids]: Ids[K]["role"] };
  body: BodyProperties;
  options?: Options;
  methods?: Methods;
  query?: Query;
  ...
  const ref = Type.Object(
    {
      type: resourceTypeSchema(type, { description, isBase }),
      ...ids,
    },
    {
      additionalProperties: isBase,
  ...
  return {
    ...
    idsOrder: idsOrder as IsBase extends true ? string[] : typeof idsOrder,
    ref,
    refOrder: refOrder as IsBase extends true ? string[] : typeof idsOrder,
    createIdKey: createId as IsBase extends true
      ? string | undefined
      : typeof createId,
    authRoles,
    body,
    methods: methods as IsBase extends true
      ? Record<string, ResourceMethodSchema>
      : typeof methods,
    options,
    query,
  };
...
export const areResourceIdsEqual = <TResource extends BaseResource>(
  ...
  return resource.idsOrder.every(([key]) => {
    if (key === undefined) {
      return false;
    }

    const keyAsId = key as keyof ResourceIds<TResource>;
    return idsA[keyAsId] === idsB[keyAsId];
  });
};
